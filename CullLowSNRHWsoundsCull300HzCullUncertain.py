...
@author: kaity
"""
# Ok so this one we are going to remove any  uncertain killer whale calls
...
model.save('Balanced_melSpec_8khz_HWcull300Hz.keras')
...
##############################################################################
# Train a model with tanh activation instead
##############################################################################
...
# Create the train and test batch loaders
train_batch_loader = Eco.BatchLoader2(h5_file, 
                           trainTest = 'train', batch_size=1, n_classes=7,    
                              minFreq=300)
...
# Create and compile model
smallResnetTanH = Eco.create_model_with_resnet(input_shape, num_classes,  actName='tanh')
...
smallResnetTanH.save('C:/Users/kaity/Documents/GitHub/Ecotype/SmallResNetTanH_Balanced_melSpec_8khz_kw_vsNot_300hz.keras')
...
val_batch_loader =  Eco.BatchLoader2(h5_file, 
                           trainTest = 'train', batch_size=1000,  n_classes=7,
                            minFreq=300)
confResults = Eco.confuseionMat(model= smallResnetTanH, 
                                val_batch_loader=val_batch_loader)
