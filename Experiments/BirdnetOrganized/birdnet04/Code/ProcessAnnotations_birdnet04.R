# Birdnet 04 — 3000 calls per class 200 replicates of each call type
rm(list = ls())
set.seed(5)
...
for (lab in all_labels) {
  ...
  if (lab %in% c("SRKW", "TKW")) {
    ...
    # Ensure at least 200 per calltype
    ...
    calltype_samples <- lapply(calltypes, function(ct) {
      ...
      slice_sample(ct_rows, n = 200, replace = (nrow(ct_rows) < 200))
    })
    ...
    n_remaining <- 3000 - nrow(ct_block_dedup)
    ...
    # Balanced 1500 Abiotic + 1500 UndBio
    abiotic_rows <- df_lab %>%
...
write.csv(balanced_data, 'DCLDE_train_birdnet04.csv')
...
ggplot(label_dataset_counts, aes(x = Labels, y = Count, fill = Provider)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Balanced Provider Composition by Label and Provider",
       x = "Label",
