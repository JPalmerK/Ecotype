...
# Label the KW pulsed calls
allAnno$isPulsedCalls =allAnno$EcotypeCertain & 
  !allAnno$CallType %in%  c('Buzz','buzz', 'EL', 'Multiple', 'Rasp','Unk', 'Whistle', 
                                                  'whistle/tone', 'W', 'whistle', 'rasp', 'Ck', '')

# Create call type labels/bins
...
# Step through the calltypes and add them to the categories
for(calType in callTypes){
  idx = which(grepl(calType, allAnno$CallType))
  allAnno$CalltypeCategory[idx] =calType}

# Create label's for the class/species datasets
allAnno$Labels = 'Background'
allAnno$Labels[allAnno$ClassSpecies == 'HW'] = 'HW'
allAnno$Labels[allAnno$Ecotype == 'NRKW'] = 'NRKW'
allAnno$Labels[allAnno$Ecotype == 'OKW'] = 'OKW'
allAnno$Labels[allAnno$Ecotype == 'SRKW'] = 'SRKW'
allAnno$Labels[allAnno$Ecotype == 'TKW'] = 'TKW'
allAnno$Labels[allAnno$Ecotype == 'SAR'] = 'SAR'
...
allAnno$HoldOut <- 'DCLDE_Train'
allAnno$HoldOut[allAnno$Provider == 'JASCO_Malahat'] <- 'JASCO_Malahat'
...
allAnno$HoldOut[allAnno$Soundfile %in% test_files & allAnno$Provider != 'JASCO_Malahat'] <- 'DCLDE_HoldOut'

table(allAnno$ClassSpecies[allAnno$HoldOut == 'DCLDE_Train'])
table(allAnno$ClassSpecies[allAnno$HoldOut == 'DCLDE_HoldOut'])

table(allAnno$Dataset[allAnno$HoldOut == 'DCLDE_Train'])
table(allAnno$Dataset[allAnno$HoldOut == 'DCLDE_HoldOut'])

# Check files are not repeated in train and hold out sections
DCLDE_train = allAnno[allAnno$HoldOut == 'DCLDE_Train',]
