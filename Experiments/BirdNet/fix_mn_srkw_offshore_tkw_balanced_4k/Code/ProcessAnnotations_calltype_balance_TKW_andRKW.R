...
TKW_calls = rbind(TKW_calls, allAnno[sample(TKWids, 4600-nrow(TKW_calls)),])
...
OKW_calls = rbind(OKW_calls, allAnno[sample(OKWids, 4600-nrow(OKW_calls), replace = TRUE),])
OKW_calls$Labels = 'OKW'

# OKW indexes that were not in the existing SRKW call dataset
NRKWids = which(allAnno$Ecotype == 'NRKW' & !allAnno$ID %in% NRKW_calls$ID)
NRKW_calls = rbind(NRKW_calls, allAnno[sample(NRKWids, 4600-nrow(NRKW_calls)),])
...
set.seed(123)
partition_HW = createDataPartition(available_HW$Provider, p = 4600 / nrow(available_HW), list = FALSE)
...
set.seed(123)
partition_BG = createDataPartition(available_BG$Provider, p = 4600 / nrow(available_BG), list = FALSE)
