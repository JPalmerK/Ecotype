...
allAnno = allAnno[!allAnno$CallType  %in%
                c('Buzz','buzz', 'EL', 'Multiple', 'Rasp','Unk', 'Whistle', 
                  'whistle/tone', 'W', 'whistle', 'rasp'),]

# Remove uncertain calls
allAnno = allAnno[which(!grepl("\\?", allAnno$CallType)),]

# General idea this time is to build each class one at a time and try to make
# it balanced with ecotypes and then negative

table(allAnno$ClassSpecies)
table(allAnno$Ecotype)

set.seed(5)
background_idx = sample(which(allAnno$KW == 0 & allAnno$ClassSpecies != 'HW'), 
                        6000, replace = FALSE)
HW_idx = sample(which(allAnno$ClassSpecies == 'HW'), 6000, replace = FALSE)
SRKW = sample(which(allAnno$Ecotype == 'SRKW'), 6000, replace = FALSE)
TKW = sample(which(allAnno$Ecotype == 'TKW'), 6000, replace = FALSE)
OKW = sample(which(allAnno$Ecotype == 'OKW'), 6000, replace = TRUE)

dataOut = allAnno[c(background_idx, HW_idx, SRKW, TKW, OKW),]

dataOut$Labels = 'Background'
dataOut$Labels[dataOut$ClassSpecies ==  'HW']<-'HW'
dataOut$Labels[dataOut$Ecotype ==  'SRKW']<-'SRKW'
dataOut$Labels[dataOut$Ecotype ==  'TKW']<-'TKW'
dataOut$Labels[dataOut$Ecotype ==  'OKW']<-'OKW'

write.csv(dataOut,
          'C:\\Users\\kaity\\Documents\\GitHub\\Ecotype\\Experiments\\BirdNet\\Backgrd_mn_srkw_tkw_okw_6k\\BirdNetAnnotations.csv')
