...
MnSUB= MnSUB[sample(1:nrow(MnSUB), 1500),]
abSUB= abSUB[sample(1:nrow(abSUB), 1500),]
UndSUB= UndSUB[sample(1:nrow(UndSUB), 1500),]
...
smallerDataset= droplevels(smallerDataset)
...
write.csv(train, 'SmallerDataONC_fix_Train.csv')
write.csv(test, 'SmallerDataONC_fix_Test.csv')


#########################################################################
# Work up the JASCO malahat data
orig = JASCO_malahat
# There is a click in there, nix it
JASCO_malahat = JASCO_malahat[JASCO_malahat$CallType != 'CK',]
JASCO_malahat = JASCO_malahat[!is.na(JASCO_malahat$Soundfile),]
JASCO_malahat$Lables = as.character(JASCO_malahat$ClassSpecies)


# Remove KWs with no ecotyp
nixIdx = which(is.na(JASCO_malahat$Ecotype) & JASCO_malahat$KW ==1)
JASCO_malahat= JASCO_malahat[-c(nixIdx),]

JASCO_malahat$Lables[which(JASCO_malahat$Ecotype == 'SRKW')]= 'SRKW'
JASCO_malahat$Lables[which(JASCO_malahat$Ecotype == 'TKW')]= 'TKW'

# Create a lookup dictionary where names are labels and values are Class
lookupDf = data.frame(Lables = unique(smallerDataset$Lables))
lookupDf$label=0

for(ii in 1:nrow(lookupDf)){
  label = unique(smallerDataset$label[smallerDataset$Lables== lookupDf$Lables[ii]])
  lookupDf$label[ii] = label
}



JASCO_malahat <- merge(JASCO_malahat,lookupDf,   by = "Lables", all.x=TRUE)
JASCO_malahat$label = as.numeric(JASCO_malahat$label)

write.csv(JASCO_malahat, 'SmallerDataset_eval_DataONC_fix_Train.csv')
