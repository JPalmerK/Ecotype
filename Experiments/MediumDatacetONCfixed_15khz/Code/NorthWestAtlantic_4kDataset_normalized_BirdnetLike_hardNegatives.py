...
valLoader =  Eco.BatchLoader_hardNegs(h5_file_validation, 
                            trainTest = 'train', batch_size=200,  n_classes=7,  
                            minFreq=0,   return_data_labels = False)

trainLoader =  Eco.BatchLoader_hardNegs(h5_fileTrainTest, 
                            trainTest = 'train', batch_size=64,  n_classes=7,  
                            minFreq=0)
testLoader =  Eco.BatchLoader_hardNegs(h5_fileTrainTest, 
...
model = Eco.create_birdnet_like_model(input_shape, num_classes)
model = Eco.compile_model(model)


history, history_hard =Eco.train_model_history_hardNeg(model, trainLoader, 
