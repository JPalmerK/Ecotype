...
ONC_anno = read.csv('E:/DCLDE/ONC/Annotations/BarkleyCanyonAnnotations_KWPWDsubset.csv')
...
kwSub = kwSub[!kwSub$CallType  %in%
                c('Buzz','buzz', 'EL', 'Multiple', 'Rasp','Unk', 'Whistle', 
                  'whistle/tone', 'W', 'whistle', 'rasp'),]


kwSub = kwSub[kwSub$HasQ == FALSE,]

# We can pull from this later to augment our dataset back up
lessCertainKWs =kwSub
...
smallerDataset = fullDataframe

set.seed(5)
for(ii in 1: nrow(SRKWCallCounts)){
  ncalls = SRKWCallCounts$Freq[ii]
  
  # if more than 100 calls then nix a random subset
  if (ncalls>120){
    
    # Index of the call types
    callTypeIdx = which(smallerDataset$CalltypeCategory == SRKWCallCounts$Var1[ii])
    idxNix = sample(callTypeIdx, length(callTypeIdx)-120)    
    
    smallerDataset <- smallerDataset[-c(idxNix), ]
  }
  
  # Also buffer some of the smaller ones
  if(ncalls<10){
    callTypeIdx = which(smallerDataset$CalltypeCategory == SRKWCallCounts$Var1[ii])
    
    # get the center times
    addIdx = sample(callTypeIdx, 20-length(callTypeIdx), replace = TRUE)
    
    # Dataset to add
    augmentedDataframe = smallerDataset[addIdx,]
    
    # Center times and durations
    centerTimes = (augmentedDataframe$FileEndSec+augmentedDataframe$FileBeginSec)/2
    durations = (augmentedDataframe$FileEndSec-augmentedDataframe$FileBeginSec)
    
    centerShift= (durations*floor(runif(length(durations),-50,50))/100)
    ...
    smallerDataset= rbind(smallerDataset, augmentedDataframe)
    
  }
  
}

# Ok now the ecotypes are a bit more balanced with 1315 NRKW, 1419 SRKW. Add
# back in the tranisents to get to 1300 

transientSubs = allAnno[allAnno$Ecotype=='TKW',] # all origional transient KWS
transientSubs = transientSubs[!is.na(transientSubs$Soundfile),]
transientSubs$CalltypeCategory = 'NA'
transientSubs$HasQ = 0
# Remove duplicated transients from the smaller dataset

transientSubs= transientSubs[!transientSubs$ID %in% smallerDataset$ID,]
# OK there are like 12k and we need about 1200

TransientProviders = as.data.frame(table(transientSubs$Provider))
#   DFO_CRP    DFO_WDA JASCO_VFPA        ONC        SIO        UAF 
#   7516        943         29        350       2207       1212 


for(ii in 1: nrow(TransientProviders)){
  ncalls = TransientProviders$Freq[ii]
  
  # if more than 100 calls then nix a random subset
  if (ncalls>200){
    
    # Index of the call types
    callTypeIdx = which(transientSubs$Provider == TransientProviders$Var1[ii])
    idxAdd = sample(callTypeIdx, 300)    
    
    smallerDataset <- rbind(smallerDataset, transientSubs[idxAdd,])
  }
  
}

# Ok now do the same for  OKWs

OffshoreSubs = allAnno[allAnno$Ecotype=='OKW',] # all origional transient KWS
OffshoreSubs = OffshoreSubs[!is.na(OffshoreSubs$Soundfile),]
OffshoreSubs$CalltypeCategory = 'NA'
OffshoreSubs$HasQ = 0
# Remove duplicated transients from the smaller dataset

OffshoreSubs= OffshoreSubs[!OffshoreSubs$ID %in% smallerDataset$ID,]
# OK there are like 12k and we need about 1200

OffshoreProviders = as.data.frame(table(OffshoreSubs$Provider))
# DFO_CRP     ONC     SIO     UAF 
#    1044     325     347     823  


for(ii in 1: nrow(OffshoreProviders)){
  ncalls = OffshoreProviders$Freq[ii]
  
  # if more than 100 calls then nix a random subset
  if (ncalls>700){
    
    # Index of the call types
    callTypeIdx = which(OffshoreSubs$Provider == OffshoreProviders$Var1[ii])
    idxAdd = sample(callTypeIdx, 700)    
    
    smallerDataset <- rbind(smallerDataset, OffshoreSubs[idxAdd,])
  }
  
}

smallerDataset$Lables = smallerDataset$Ecotype

# Add in KW ecotypes that aren't already there 
kwSubExcluded = lessCertainKWs[!lessCertainKWs$ID %in% unique(smallerDataset$ID), ]
labels = unique(smallerDataset$Lables)

# Add enough for each dataset to be 4000 examples long
for(ii in 1:length(labels)){
  
  # number of datapoints to add
  nToadd = 4000-sum(smallerDataset$Lables==labels[ii])
  
  # set a flag to use replace if the output is shorter than the existing list
  replaceFlag = nToadd> length(which(kwSubExcluded$Ecotype== labels[ii]))
  
  idxAdd = sample(which(kwSubExcluded$Ecotype == labels[ii]), nToadd,
                  replace = replaceFlag)
  
  
  # If using replace, do data augmentation
  # Also buffer some of the smaller ones
  if(replaceFlag == TRUE){

    # Dataset to add
    augmentedDataframe = kwSubExcluded[idxAdd,]
  ...
  augmentedDataframe$HasQ = NA
  augmentedDataframe$Lables = labels[ii]
  augmentedDataframe$CalltypeCategory = NA
...
##########################################################################
# Add in humpbacks, abiotica,  and random signals 
###########################################################################
allAnno$CalltypeCategory = 'NA'
allAnno$HasQ = 0

MnSUB = allAnno[allAnno$ClassSpecies == 'HW',]
abSUB = allAnno[allAnno$ClassSpecies == 'AB',]
UndSUB = allAnno[allAnno$ClassSpecies == 'UndBio',]

MnSUB= MnSUB[sample(1:nrow(MnSUB), 4000),]
abSUB= abSUB[sample(1:nrow(abSUB), 4000),]
UndSUB= UndSUB[sample(1:nrow(UndSUB), 4000),]

MnSUB$Lables = 'HW'
abSUB$Lables = 'AB'
UndSUB$Lables = 'UndBio'



smallerDataset= rbind(smallerDataset, MnSUB, abSUB, UndSUB)

table(smallerDataset$Lables)
smallerDataset$label = as.numeric(as.factor(smallerDataset$Lables))-1
...
train_indices <- createDataPartition(smallerDataset$Lables, p = 0.8, list = FALSE)
train <- smallerDataset[train_indices, ] %>%mutate(traintest = "Train")
test <- smallerDataset[-train_indices, ] %>% mutate(traintest = "Test")


write.csv(train, 'C:\\Users\\kaity\\Documents\\GitHub\\Ecotype\\Experiments\\BirdNet\\Birdnet7Class_ONC_Larger\\Data\\SmallerDataONC_fix_Train_4k.csv')
write.csv(test, 'C:\\Users\\kaity\\Documents\\GitHub\\Ecotype\\Experiments\\BirdNet\\Birdnet7Class_ONC_Larger\\Data\\SmallerDataONC_fix_Test_4k.csv')
